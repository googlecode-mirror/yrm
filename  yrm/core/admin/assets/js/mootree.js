...
var MooTreeControl = new Class({
	...
	initialize: function(config, options) {
		...
		this.theme = config.theme || 'components/com_yos_resources_manager/assets/images/mootree.gif';
		
		this.loader = config.loader || {icon:'components/com_yos_resources_manager/assets/images/mootree_loader.gif', text:'Loading...', color:'#a0a0a0'};
...
var MooTreeNode = new Class({
	...
	initialize: function(options) {
		...
		//yos hack begin
		this.checked    = options.checked;      // the checkbox value
		this.affected   = options.affected;     //affected domain
		this.published  = options.published;    // published value
        this.elink      = options.elink;        //edit link url
        this.etitle     = options.etitle;       //edit link title
		//yos hack end
		...
		this.div = {
			...
			//yos hack begin 
			//to add checkbox
			checkbox: new Element('div').addClass('yos_tree_checkbox_container'),
			//yos hack end
		...
		//yos hack begin to add checkbox
		...
		//yos hack begin to handle checkbox clicked
		this.div.checkbox._node = this;
		this.div.checkbox.onclick = function() {
			var arr_inputbox = this.getChildren('input');
			this._node.checked = arr_inputbox[0].checked
			this._node.clickcheckbox(this._node.checked);
		}
		//yos hack end
	...
	*/
	...
	update: function(recursive, invalidated) {
		...
		if (draw) {
			...
			if(this.id){
				//adding check box begin
                //update the checkbox
                var checked = this.checked == 'true' | this.checked == true ? 'checked' : '';
                var checkboxId = this.id ? 'yos_cb_' + this.id : 'yos_cb_0';
                var myElement = new Element('input', {
                        'id':checkboxId,
                        'class': 'yos_tree_checkbox',
                        'type':'checkbox',
                        'name':'cid[]',
                        'value':this.id,
                        'checked':checked,
                ...
                this.div.checkbox.empty();
                this.div.checkbox.adopt(myElement);
                //adding check box end
				
				//adding affected image begin
				if(this.affected == 'F' || this.affected == 'B' || this.affected == 'BF'){
                     //adding separate begin
                     var mySeparator1 = new Element('span', {
                    'class': 'yos_tree_separator',
                    'html': '&nbsp;'
                    });
					x.grab(mySeparator1, 'bottom');
					//adding separate end
                    
					var affectedId = 'yos_p_' + this.id;
					switch(this.affected){
						case 'B':
							var affectedUrl = 'components/com_yos_resources_manager/assets/images/affected_B.png';
							break;
						case 'BF':
							var affectedUrl = 'components/com_yos_resources_manager/assets/images/affected_BF.png';
							break;
						default:
							var affectedUrl = 'components/com_yos_resources_manager/assets/images/affected_F.png';
							break;
					}
					
					var myElement2 = new Element('img', {
						'id': affectedId,
						'src': affectedUrl, 
						'class': 'yos_tree_img',
						'title': 'Affected domain'
						});
					x.grab(myElement2, 'bottom');
				}
				//adding affected image end
				
				//adding published image begin
				if(this.published == 0 || this.published == 1){

					//adding separate begin
                     var mySeparator2 = new Element('span', {
                    'class': 'yos_tree_separator',
                    'html': '&nbsp;'
                    });
					x.grab(mySeparator2, 'bottom');
					//adding separate end
					
					var publishedId = 'yos_p_' + this.id;
					var publishedUrl = this.published == 1 ? 'components/com_yos_resources_manager/assets/images/tick.png' : 'components/com_yos_resources_manager/assets/images/publish_x.png';
					var myElement1 = new Element('img', {
						'id': publishedId,
						'src': publishedUrl, 
						'class': 'yos_tree_img',
						'title': 'status'
						});
					x.grab(myElement1, 'bottom');
				}
				//adding published image end

                //adding edit image link
				if(this.etitle != null && this.elink != null){
					//adding separate begin
                     var mySeparator3 = new Element('span', {
                    'class': 'yos_tree_separator',
                    'html': '&nbsp;'
                    });
					x.grab(mySeparator3, 'bottom');
					//adding separate end
                    var eImgId = 'yos_e_img_' + this.id;
					var eImgUrl = 'components/com_yos_resources_manager/assets/images/edit.png';
					var myElement3 = new Element('img', {
						'id': eImgId,
						'src': eImgUrl,
						'class': 'yos_tree_e_img',
						'title': this.etitle
						});
					x.grab(myElement3, 'bottom');
				}
				//adding edit image link
			}
	...
	//yos hack begin
	clickcheckbox: function(checked) {
		this.checked = checked;
		var arr_inputbox = this.div.checkbox.getChildren('input');
		arr_inputbox[0].checked = this.checked;
		this.nodes.each(function(node){
			node.clickcheckbox(checked);
		});
	},
	//yos hack begin
	...
	*/
	...
	getImg: function(name) {
		...
		if ((i == 9) || (i == 10) || (i == 11)){
			//return '<input type="checkbox" class="yos_tree_checkbox" name="" />';
			return '';
		}
		...
		for (var i=0; i<n.length; i++) if (n[i].tagName == 'node') {
			...
			for (var t=0; t<a.length; t++) {
				switch (a[t].name) {
					...
					case 'color':
					case 'open':
					//yos hack to add new attributes begin
					case 'checked':
					...
					case 'etitle':
						opt[a[t].name] = a[t].value;
						break;
					//yos hack to add new attributes end
					default:
